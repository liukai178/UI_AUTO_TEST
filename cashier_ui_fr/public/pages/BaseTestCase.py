...
class BaseTestCase(unittest.TestCase):
    ...
    @classmethod
    # element是个元组，里面有定位方式和要定位的元素
    def find_elements(cls, element):
        # 元组第一个索引为方式，第二个索引为该方式对应的元素
        type = element[0]
        value = element[1]
        try:
            if type == "id" or type == "Id" or type == "ID":
                elem = cls.driver.find_elements_by_id(value)
            elif type == "name" or type == "Name" or type == "NAME":
                elem = cls.driver.find_elements_by_name(value)
            elif type == "class" or type == "Class" or type == "CLASS":
                elem = cls.driver.find_elements_by_class_name(value)
            elif type == "xpath" or type == "Xpath" or type == "XPATH":
                elem = cls.driver.find_elements_by_xpath(value)
            elif type == "css" or type == "Css" or type == "CSS":
    ...
    def get_text(cls,locator):
        text = BaseTestCase.find_element(locator).text
        return text

    @classmethod
    #每次跑完用例回到首页
    def goto_home(cls):
        home = BaseTestCase.find_element(P.homevalue)
        BaseTestCase.click(home)

    @classmethod
    # 从个人中心回到首页
    def goto_homeself(cls):
        homeself = BaseTestCase.find_element(P.homevalueself)
        BaseTestCase.click(homeself)
    ...
    @classmethod
    #显式等待
    def webdriverwait(cls,locator):
        return WebDriverWait(cls.driver, 5).until(expected_conditions.visibility_of_element_located(locator))

    @classmethod
    #鼠标悬停
    def moveto(cls,locator):
        return ActionChains(cls.driver).move_to_element(locator).perform()
