...
def aoto_run():
    ...
    # suite.addTest(loader.loadTestsFromName("testcases.Test_Self_Center1.Test_Self_Center.test001_self_center"))
    # print(suite)

    # 2.使用discover自动搜索用例
    discover = unittest.defaultTestLoader.discover(CASEDIR, 'test_*.py')
    ...
    with open(filename, 'wb')as f:
        ...
        runner = HTMLTestReportCN(stream=f,title=u"收银系统后台UI自动化测试报告",
                                description=u"用例执行情况如下",tester=u"刘凯")
        #运行执行器的run函数（参数为装满用例的套件）
        runner.run(discover)

def Send_mail():
    send_email(filename,u"收银系统后台UI自动化测试报告")
